...
ft_full_tokens_emb = np.load('embedding/train/LaserEmb_full.npy', allow_pickle=True)
...
train_composite_with_token = [train_attr_x, pages_class, pages_query, train_tag_x]
...
out =crf(model)
model = Model([input_ft_embedding, input_class, input_query, input_tag_information], out)
...
def prepare_for_testing(test_X_raw, test_y_raw): #ft-bert -no chunks
    ...
    test_composite_input = [test_ft_emb, test_pages_class, test_pages_query, test_tag_x]
